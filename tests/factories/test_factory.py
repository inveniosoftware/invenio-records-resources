...
def test_record_class_create():
    ...
    # check schema field
    assert rec_type.record_cls.schema.value == \
...
def test_gen_same_name_classes():
    ...
    with pytest.raises(RecordTypeFactoryError):
...
def test_resource_class_create():
    ...
    assert rec_type.resource_config_cls.__name__ == \
           'ResourceTestResourceConfig'

    assert rec_type.resource_config_cls.list_route == 'api/resourcetests'
    assert rec_type.resource_config_cls.item_route == \
           'api/resourcetests/<pid_value>'

    links_schema_class = rec_type.resource_config_cls.links_config["record"]
    search_links_schema_class = \
        rec_type.resource_config_cls.links_config["search"]

    assert links_schema_class.__name__ == "ResourceTestLinksSchema"
    assert search_links_schema_class.__name__ == \
           "ResourceTestSearchLinksSchema"
    ...
    assert links_schema_class.self.template.uri == \
           URITemplate('api/resourcetests/{pid_value}').uri
    assert search_links_schema_class.self.template.uri == \
...
def test_optional_schema_path():
    rec_type = RecordTypeFactory(
        'OptionalSchemaPath', RecordSchema,
...
def test_optional_version():
    rec_type = RecordTypeFactory(
        'OptionalVersion', RecordSchema,
        schema_version='1.1.0')
    assert rec_type.record_cls.schema.value == \
           'https://localhost/schemas/optionalversions' \
           '/optionalversion-v1.1.0.json'
    assert rec_type.record_cls.index._name == \
           'optionalversions-optionalversion-v1.1.0'


def test_optional_index_name():
    rec_type = RecordTypeFactory(
        'OptionalIndex', RecordSchema,
        index_name='myindices-index-v1.0.0')

    assert rec_type.record_cls.index._name == \
           'myindices-index-v1.0.0'


def test_optional_endpoint_route():
    rec_type = RecordTypeFactory(
        'OptionalEndpoint', RecordSchema,
        endpoint_route='api/customroute')
...
def test_optional_service_params():
    ...
    rec_type = RecordTypeFactory(
        'SearchFacets', RecordSchema,
        search_facets_options=search_facets_options,
        service_components=[CustomComponent])

    assert rec_type.service_config_cls.search_facets_options == \
           search_facets_options

    assert rec_type.service_config_cls.components == \
