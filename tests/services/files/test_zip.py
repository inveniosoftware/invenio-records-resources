...
def test_zip_listing(identity_simple, file_service, record_with_zip):
    ...
    assert entries == [{'key': 'a.txt', 'size': 24, 'compressed_size': 24, 'mimetype': 'text/plain', 'crc': 3057564182,
                       ...
                       {'key': 'b.txt', 'size': 24, 'compressed_size': 24, 'mimetype': 'text/plain', 'crc': 3057564182,
                        'links': {
                            'content': f"https://127.0.0.1:5000/api/mocks/{recid}/files/testzip.zip/container/b.txt"}},
                       {'key': 'c.txt', 'size': 24, 'compressed_size': 24, 'mimetype': 'text/plain', 'crc': 3057564182,
                        'links': {
                            'content': f"https://127.0.0.1:5000/api/mocks/{recid}/files/testzip.zip/container/c.txt"}},
                       {'key': 'd.txt', 'size': 24, 'compressed_size': 24, 'mimetype': 'text/plain', 'crc': 3057564182,
                        'links': {
...
def test_read_zip(identity_simple, file_service, record_with_zip):
    recid = record_with_zip["id"]
    with file_service.open_from_container(identity_simple, recid, "testzip.zip", "a.txt") as f:
        data = f.read()
...
def test_zip_extraction(identity_simple, file_service, record_with_zip):
    recid = record_with_zip["id"]
    extracted = file_service.extract_from_container(identity_simple, recid, "testzip.zip", "a.txt")
    extracted_data = extracted.send_file()
    assert (
            extracted_data.get_data() == b'Hello world from a.txt.\n'
    )
